...
public class ListMemberService {
	...
	//페이지정보
	public ListMemberPage getMemberListPage(int pageNo) {
		try {
			Connection conn = JdbcUtil.getConnection();
			
			//전체 게시글수 조회
			int totalCount = MemberListDAO.selectCount(conn);
			...
			List<Memberlist> content = 
				MemberListDAO.select(conn, (pageNo-1)*size, size);
			
			//페이지정보가 포함된 내용을 리턴을 해줘야 한다
			return new ListMemberPage(totalCount, pageNo, size, content);
