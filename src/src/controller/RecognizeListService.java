...
//p650
//DAO와 연동
public class RecognizeListService {
	
	private InfoDAO	info = new InfoDAO();
	private int size = 5;
	
	public RecognizePage getArticlePage(int pageNo) {
		try {
			...
			List<Recognize> content = InfoDAO.select(conn, (pageNo-1)*size, size);
			
			//페이지정보가 포함된 내용을 리턴을 해줘야 한다
			return new RecognizePage(total, pageNo, size, content);
	...
	public RecognizePage getRecognizePage(int pageNo) {
