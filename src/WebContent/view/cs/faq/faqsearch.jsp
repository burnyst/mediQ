...
<jsp:include page="../../header.jsp"></jsp:include>
	FaqSearchPage=${faqSearchPage}<br/>
	*총페이지수 ${faqSearchPage.searchtotal}건<br/>
	현재페이지 : ${faqSearchPage.currentPage}/총페이지수 ${faqSearchPage.totalPages }
<h1>FAQ</h1>
<hr/><br/>
<div class="page_body">
    <div class="board_search">
        <select name="category" id="category" onchange="if(this.value) location.href=(this.value);">
            ...
            <c:if test="${faqSearchPage.category == null}">
        ...
        <form name="fsearch" id="fsearch" method="get" action="faqsearch.do?category=${faqSearchPage.category}">
    	...
    	<table cellpadding="0" cellspacing="0" board="1" width="100%" class="result">
    		<thead>
				...
				<c:forEach var="faq" items="${faqSearchPage.searchlist}">
	    			<tr class="row">
	    				<td>${faq.sn}</td>
	    				<td><a href="faqscount.do?sn=${faq.sn}&category=${faq.category}">${faq.title}</a></td>
	    				<td>${faq.mid}</td>
	    				<td>${faq.rdate}</td>
	    				<td>${faq.category}</td>
	    				<td>${faq.vcount}</td>
